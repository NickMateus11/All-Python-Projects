    ...
    class obj_enemy(object):
        ...
        def endGame():
            while True:
                ...
                textWin = font.render("YOU LOSE", True, pygame.Color('white'))
                surface.blit(textWin, [75, 150])
                font = pygame.font.SysFont("monospace", 20)
                textRe=font.render("HOLD Enter To Restart",True,pygame.Color('red'))
                surface.blit(textRe, [65, 200])
                
                pygame.display.flip()
                key=pygame.key.get_pressed()
                for event in pygame.event.get(): # User did something
                    if key[pygame.K_RETURN]==True: # If user clicked close
                        GAME()
