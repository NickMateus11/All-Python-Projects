...
class new_block():
    ...
    def __init__(self,winx,winy,x,y):
        self.winx = winx
        self.winy = winy
        self.dimension = 22
    ...
    def farthest_left(self):
        left_index = 0
        for i in range(1,4):
            if self.rect[i].x < self.rect[left_index].x:
                left_index = i
        return left_index
    
    def farthest_right(self):
        right_index = 0
        for i in range(1,4):
            if self.rect[i].x > self.rect[right_index].x:
                right_index = i
        return right_index
    ...
    def get_lowest_index(self):         
        lowest = 0
        for i in range(1,4):
            if self.rect[i].y > self.rect[lowest].y:#lowest is actually highest y val
                lowest = i
        return lowest
    ...
    def is_collision(self,piece_list):
        for i in range(4):
            for piece in piece_list:
                for j in range(len(piece.rect)):
                    if self.rect[i].x == piece.rect[j].x and self.rect[i].y == piece.rect[j].y :
                        return True               
...
def validate_move(piece_list,move_type,move_val): #reverts move if invalid
    ...
    farthest_left = piece_list[-1].farthest_left()
    farthest_right = piece_list[-1].farthest_right()
    if piece_list[-1].rect[farthest_left].x < 150 or piece_list[-1].rect[farthest_right].x >= 600-150: #wall collisions
            if move_type == piece_list[-1].rotate:
                if piece_list[-1].rect[farthest_left].x < 150:
                    max_blocks_over = int((150 - piece_list[-1].rect[farthest_left].x)//piece_list[-1].block_offset)
                    direction = 1
                else:
                    max_blocks_over = int((piece_list[-1].rect[farthest_right].x - (600-150))//piece_list[-1].block_offset) + 1
                    direction = -1
                prevx = piece_list[-1].rect[0].x
                piece_list[-1].move_x(piece_list[-1].block_offset*direction*max_blocks_over)
                validate_move(piece_list,piece_list[-1].move_x,piece_list[-1].block_offset*direction*max_blocks_over)
                if prevx == piece_list[-1].rect[0].x:
                    move_type(-move_val)
            else: move_type(-move_val)
...
def is_tetris(piece_list):
    ...
    for i in range(piece_list[0].winy//piece_list[0].block_offset):
        rect_checkers.append(pygame.Rect(150,i*piece_list[0].block_offset,piece_list[0].dimension,piece_list[0].dimension))
    for j in range((piece_list[0].winx-300)//piece_list[0].block_offset):
...
def main():
    ...
    FPS = 60
    timer = 0   
    piece_list = [new_block(winx,winy,winx//2 - 25,0)]
    fall_offset = piece_list[0].block_offset
    shift_offset = piece_list[0].block_offset
    current = -1
    block_count = 0
    wait_time = 0.5
    bounds = [[150,43],[150,winy-48],[winx-150,winy-48],[winx-150,43]]
    ...
    while True:        
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                elif event.key == pygame.K_r: #rotate clockwise
                    piece_list[current].rotate(1)
                    validate_move(piece_list,piece_list[current].rotate,1)
                elif event.key == pygame.K_SPACE:
                    falling = True
                    while falling:
                        currenty = piece_list[current].rect[0].y
                        piece_list[current].move_y(fall_offset)
                        validate_move(piece_list,piece_list[current].move_y,fall_offset)
                        if currenty == piece_list[current].rect[0].y or piece_list[current].get_lowest() >= winy-50:
                            falling = False
                        
        if pygame.key.get_pressed()[pygame.K_DOWN]:
            wait_time = 0.1
        else: wait_time = 0.5 - (block_count//8)*0.075
        ...
        for piece in piece_list:
            if piece_list.index(piece) == len(piece_list)-1:
                ...
                if piece.is_collision(piece_list[:current]) or piece_list[current].rect[piece_list[current].get_lowest_index()].y >= winy-50:
                    ...
                    piece_list.append(new_block(winx,winy,winx//2 - 25,0))
                    block_count += 1
        ...
        if timer > 0: timer -= 1
            ...
            timer = FPS * wait_time//1 if wait_time > 0.1 else FPS*0.1
        clock.tick(FPS)
        pygame.display.update()
